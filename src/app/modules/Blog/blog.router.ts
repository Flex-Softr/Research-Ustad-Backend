...
router.post(
  ...
  (req: Request, res: Response, next: NextFunction) => {
    console.log('Blog POST - Original body:', req.body);
    console.log('Blog POST - File:', req.file);
    
    if (req.body.data) {
      req.body = JSON.parse(req.body.data);
      console.log('Blog POST - Parsed body:', req.body);
      
      if (req.file) {
        // Set the full URL for the uploaded image
        const baseUrl = `http://localhost:${config.port}`;
        req.body.imageUrl = `${baseUrl}/upload/${req.file.filename}`;
        console.log('Blog POST - Final imageUrl:', req.body.imageUrl);
      }
    }
    next();
  },
