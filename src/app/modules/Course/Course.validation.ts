...
const courseValidationPost = z.object({
  body: z
    .object({
      ...
      enrollLink: z.string().url('Enroll link must be a valid URL'),
      imageUrl: z.string().min(1, 'Image URL is required'),
      instructors: z
        .array(instructorValidation)
    ...
    .refine(
      (data) => {
        if (
          ...
          (!data.offlineLocation || data.offlineLocation.trim() === '')
        ) {
          return false;
        }
        return true;
...
const courseValidationUpdate = z.object({
  body: z
    .object({
      ...
      enrollLink: z.string().url('Enroll link must be a valid URL').optional(),
    ...
    .refine(
      (data) => {
        if (data.capacity !== undefined && data.enrolled !== undefined) {
          return data.capacity >= data.enrolled;
        }
