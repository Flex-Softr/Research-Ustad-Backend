...
export class UserService {
  ...
  if (payload.role === 'superAdmin') {
    ...
    const userData: Partial<TUser> = {
      ...
      image: payload.image || "https://www.shutterstock.com/image-vector/default-avatar-profile-icon-social-600nw-1906669723.jpg",
 ...
 */
   ...
   */
  static async getResearchMembersByDesignation(designation: string): Promise<TUser[]> {
   ...
   */
  static async addPaperToAuthors(paperId: Types.ObjectId, authors: Array<{ name: string; email?: string } | string>): Promise<void> {
    const session = await mongoose.startSession();
    
    try {
      await session.startTransaction();
      
      // Filter authors with emails (handle both old and new formats)
      const authorsWithEmails = authors.filter(author => {
        if (typeof author === 'string') {
          return false; // Old format - no email
        }
        return author.email && author.email.trim() !== '';
      }) as Array<{ name: string; email?: string }>;
      
      if (authorsWithEmails.length === 0) {
        console.log('📝 No authors with emails found, skipping publication linking');
        return;
      }
      
      // Get all author emails
      const authorEmails = authorsWithEmails.map(author => author.email!.toLowerCase());
      
      console.log(`🔍 Looking for users with emails:`, authorEmails);
      
      // Get total user count for comparison
      const totalUsers = await User.countDocuments().session(session);
      console.log(`📊 Total users in database: ${totalUsers}`);
      
      // Find all users whose email matches any of the author emails
      const matchingUsers = await User.find({
        email: { $in: authorEmails }
      }).session(session);
      
      console.log(`✅ Found ${matchingUsers.length} matching users out of ${totalUsers} total users`);
      console.log(`📈 Match rate: ${((matchingUsers.length / totalUsers) * 100).toFixed(2)}%`);
      ...
      for (const user of matchingUsers) {
        ...
        const paperExists = user.publications.some(pubId => pubId.equals(paperId));
        if (!paperExists) {
          ...
          console.log(`📄 Added paper ${paperId} to user ${user.email}'s publications`);
        } else {
          console.log(`📄 Paper ${paperId} already exists in user ${user.email}'s publications`);
        }
      }
      
      await session.commitTransaction();
      console.log(`🎉 Successfully linked paper ${paperId} to ${matchingUsers.length} authors`);
  ...
  static async removePaperFromAuthors(paperId: Types.ObjectId): Promise<void> {
    ...
    try {
      ...
      console.log(`🔍 Found ${usersWithPaper.length} users with paper ${paperId}`);
      ...
      for (const user of usersWithPaper) {
        if (user.publications) {
          ...
          console.log(`🗑️ Removed paper ${paperId} from user ${user.email}'s publications`);
        }
      }
      
      await session.commitTransaction();
      console.log(`🎉 Successfully removed paper ${paperId} from ${usersWithPaper.length} authors`);
   ...
   */
  static async updatePaperAuthors(paperId: Types.ObjectId, newAuthors: Array<{ name: string; email?: string } | string>): Promise<void> {
    const session = await mongoose.startSession();
  ...
  static async getUserPublications(userId: string): Promise<any[]> {
    const user = await User.findById(userId).populate({
      path: 'publications',
      select: 'title authors journal year status isApproved visitLink abstract keywords citations researchArea funding createdAt'
    });
...
export const UserServices = {
  // User creation
  createResearchMembar: (file: any, password: string, payload: Partial<TUser>) => 
  ...
  getResearchMembersByDesignation: (designation: string) => UserService.getResearchMembersByDesignation(designation),
  getSingleResearchMember: (id: string) => UserService.getUserById(id),
  getSingleResearchMemberByEmail: (email: string) => UserService.getUserByEmail(email),
  
  // User management
  userToadmin: (id: string) => UserService.toggleUserRole(id),
  deleteUser: (id: string, requestingUserId?: string) => UserService.deleteUser(id, requestingUserId),
  searchUsers: (query: string) => UserService.searchUsers(query),
  
  // User updates
  updateUser: (id: string, payload: Partial<TUser>) => UserService.updateUser(id, payload),
  updateUserByEmail: (email: string, payload: Partial<TUser>) => UserService.updateUserByEmail(email, payload),
  updateResearchMember: (id: string, payload: Partial<TUser>) => UserService.updateUser(id, payload),
  updateResearchMemberByEmail: (email: string, payload: Partial<TUser>) => UserService.updateUserByEmail(email, payload),
  
  // SuperAdmin management
  replaceSuperAdmin: (newSuperAdminId: string, requestingUserId?: string) => UserService.replaceSuperAdmin(newSuperAdminId, requestingUserId),
  ...
  deleteResearchMember: (id: string, requestingUserId?: string) => UserService.deleteUser(id, requestingUserId),
};
